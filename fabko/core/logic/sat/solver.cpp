...
struct sat_execution_context {
    /**
   * Assignment currently on trial for satisfiability.
   */
    assignment_bitset<> cur_assignment{};

    /**
   * Assignment that has been registered as being a potential result for the SAT resolution.
   * If this vector is filled with at least one value, it means the SAT resolved.
   *
   * It is possible for the SAT solver to resolve as many times as required (or until no new resolution is found).
...
struct solver::sat_impl {
   ...
   */
    [[nodiscard]] solver_status solve_sat(int requested_sat_solution) {
        const auto num_var = context.cur_assignment.nb_vars();

        // The list of a binary flag will keep track of what values for which variables we have tried so far.
        ...
        while (requested_sat_solution > 0 || requested_sat_solution <= -1) {
            ...
            if (!assignment_happened) {
                // var cannot continue backtracking. solver cannot find additional solutions.
                if (var == 0) {
            ...
            // the latest variable has been successfully set
            else if (var == num_var) {
                log_info(
                    "SAT execution -- end - solution::{}",
                    fmt::join(to_literals(context.cur_assignment) | std::ranges::views::transform([](const auto& lit) {
                                  return fmt::format(" {}[{}] ", lit.var(), bool(lit));
                              }),
                              "|"));

                // store the current cur_assignment result (as it is a valid result)
   ...
   */
    [[nodiscard]] bool try_assign_variable(std::vector<int>& attempt_flags, variable var) {
        if (var == 0) {
            return false;
        }
        ...
        for (const bool attempt : {false, true}) {
            ...
            if (static_cast<bool>((attempt_flags[var] >> static_cast<int>(attempt)) & 1)) {
            ...
            log_debug("solve::try_assign_variable -- var::{} - attempt::{} - attempt_value::{}", var, attempt,
                      attempt_flags[var]);
            ...
            log_debug("solve::try_assign_variable -- assign var::{}[{}] - success :: {}", var, attempt,
                      watch_list_update_success);
   ...
   *
   * @details:
   *    For each literal, clauses are watching them. When updating the watchlist for a specific var
   *    we go through all the clauses watching the updated var. An alternative is looked for each of the clauses.
   *
   ...
   *    False otherwise (if false is returned, then the assignment is contradicting the resolvability and a backtracking should start)
   */
    [[nodiscard]] bool update_watchlist(variable var, bool assign) {
        ...
        auto rm_when_find_alternative =
            clauses_watching | std::ranges::views::take_while([this, var](std::size_t index_clause) {
                const auto& clause_watching = clauses[index_clause];

                return std::ranges::any_of(clause_watching.get(), [this, &index_clause, var](const literal& lit) {
                    const variable alt_var = lit.var(); // alternative to check

                    log_trace("solve::update_watchlist::take_while::any_of -- clause {} -- lit::{}[{}] -- "
                              "(check_alternative) ",
                              index_clause, alt_var, static_cast<bool>(lit));
                    log_trace("solve::update_watchlist::take_while::any_of -- is_assigned(alt_var)::{} -- "
                              "is(alt_var, bool(alt_var))::{}",
                              context.cur_assignment.is_assigned(alt_var),
                              context.cur_assignment.is(alt_var, static_cast<bool>(lit)));

                    if (!context.cur_assignment.is_assigned(alt_var)
                        || context.cur_assignment.is(alt_var, static_cast<bool>(lit))) {
                        if (std::ranges::none_of(watchlist[lit.value()],
                                                 [index_clause](std::size_t i) { return i == index_clause; })) {
        ...
        log_trace("solve::update_watchlist -- removal of elements in watchlist of {} : remove::{}",
                  clauses_watching.size(), number_to_remove);
        clauses_watching.erase(std::begin(clauses_watching), std::begin(clauses_watching) + number_to_remove);
...
std::string to_string(const literal& lit) { return fmt::format("{}[{}]", lit.var(), bool(lit)); }
...
solver::solver(fabko::sat::solver_config config) : _pimpl(std::make_unique<sat_impl>(std::move(config))) {
    reset_solver();
...
void solver::reset_solver() {
    _pimpl->context                      = {};
    _pimpl->config.flags.status_solver   = static_cast<unsigned>(solver_status::BUILDING);
    _pimpl->config.flags.previous_status = static_cast<unsigned>(solver_status::BUILDING);
}
...
void solver::add_variables(std::size_t number_to_add) {
    ...
    log_info("add_variables :: number_to_add {}. currently {} variables in sat", number_to_add,
             _pimpl->context.cur_assignment.nb_vars());

    for (variable to_add_in_watchlist :
         std::ranges::views::iota(variable{1}, _pimpl->context.cur_assignment.nb_vars() + 1)) {
        auto lit                          = literal(to_add_in_watchlist);
...
void solver::add_clause(clause clause_literals) {
    ...
    auto watcher = variable_watched{clause_literals.get(0).var(), std::nullopt};
...
solver_status solver::solving_status() const { return solver_status(unsigned(_pimpl->config.flags.status_solver)); }

void solver::solve(int requested_sat_solution) {
    log_info("solve -- {} requested_solutions", requested_sat_solution);

    if (requested_sat_solution == 0) {
        _pimpl->config.flags.status_solver = static_cast<unsigned>(solver_status::UNSAT);
        return;
    }

    _pimpl->config.flags.status_solver = static_cast<unsigned>(solver_status::SOLVING);
    _pimpl->config.flags.status_solver = static_cast<unsigned>(_pimpl->solve_sat(requested_sat_solution));
}
...
std::vector<sat_result> solver::results() const {
    ...
    for (const auto& result : _pimpl->context.valid_result_assignments) {
        auto lit_res = fabko::ranges::to_vector(
            ...
            | std::views::transform([&result](sat::variable var) { return literal{var, result.is_true(var)}; }));
...
std::size_t solver::nb_variables() const { return _pimpl->context.cur_assignment.nb_vars(); }

std::size_t solver::nb_clauses() const { return _pimpl->clauses.size(); }
